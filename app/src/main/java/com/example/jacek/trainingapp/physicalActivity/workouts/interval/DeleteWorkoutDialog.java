...
public class DeleteWorkoutDialog extends AppCompatDialogFragment
{
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState)
    {
        ...
        builder.setTitle("Usunąć zestaw?")
                .setMessage("Zestaw zniknie z listy")
                .setPositiveButton("Tak", new DialogInterface.OnClickListener()
                {
                    @Override
                    public void onClick(DialogInterface dialog, int which)
                    {
                        listener.onDeleteWorkoutClick(getArguments().getInt(Utils.POSITION));
                    }
                })
    ...
    public void onAttach(Context context)
    {
        ...
        try
        {
            listener = (DeleteWorkoutDialogListener)context;
        }
        catch (ClassCastException e)
        {
            throw new ClassCastException(context.toString()+" must implement DeleteWorkoutDialogListener");
        }
